...
**Date:** 2026-07-27T07:36:27.641Z

**Decision:** **GATE FAIL** — gated on **txvalid** steady P95 = 200.57133299999987ms vs gate ≤ 200ms.

## What was measured

2000 pre-signed txs were fired round-robin across 40 independent lanes at a sustained 200 TPS (closed-loop(50,snapshot)). Signing is off the hot path, so the rig genuinely *offers* the target rate — any remaining ceiling is the node's. Two latencies are correlated per tx: **TxValid** (node validated the state transition locally) and **SnapshotConfirmed** (settled in a signed snapshot).

| Metric | Value |
|---|---|
| Target TPS | 200 |
| Loop mode | closed-loop(50,snapshot) |
| Lanes × chain | 40 × 50 = 2000 |
| Offered (submit) | 2000 @ 108.5 TPS |
| Node-validated (TxValid) | 2000 @ 108.5 TPS |
| TxInvalid (logic reject — **gated**) | 0 |
| Stale-input race (rig timing — **excluded from gate**) | 0 |
| Confirmed (in snapshot) | 2000 @ 108.5 TPS |
| Snapshots observed | 118 |
| Avg tx / snapshot | 16.9 |
| **TxValid latency (matching) — steady P50/P95/P99/max** | **124.29799999999886 / 200.57133299999987 / 225.2857499999991 / 258.1130000000012 ms** (n=1179/2000) |
| SnapshotConfirmed latency (settlement) — steady P50/P95/P99/max | 385.9813750000012 / 470.5229580000005 / 491.6936249999999 / 521.8482910000002 ms (n=1179/2000) |
| Saturated? | false |
...
❌ **FAIL** — txvalid steady P95 200.57133299999987ms > 200ms. Locate the ceiling via offered-vs-validated above.
...
Machine-readable summary: `2.2.0-25b8bd2b/macos-arm64-native/perp-state/200tps-10s-40lanes-chained-closed50-snapshot/run-01/summary.json` (under `results/`).
